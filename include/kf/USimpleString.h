...
namespace kf
{
    ...
    inline bool USimpleString::matches(_In_ const USimpleString& expression) const
    {
        ...
        const auto& pattern = expression.m_str;
        constexpr auto kStarSymbol = L'*';
        constexpr auto kQuestionMarkSymbol = L'?';

        int starPos = -1;
        int patternPos = 0;

        for (int currentPos = 0; currentPos < charLength(); currentPos++)
        {
            if (patternPos >= expression.charLength())
            {
                // If we are at the end of the pattern, but not at the end of the string, it does not match
                return false;
            }

            // '*' is the last symbol in the pattern, so it matches everything
            if (pattern.Buffer[patternPos] == kStarSymbol && patternPos == expression.charLength() - 1)
            {
                return true;
            }

            if (pattern.Buffer[patternPos] == kStarSymbol)
            {
                patternPos++;
                starPos = currentPos;
            ...
            else if (starPos != -1)
            {
                ...
                {
                    nextWildCPos = min(asteriskIndex, questionMarkIndex);
                }
                else if (asteriskIndex != -1)
                {
                    nextWildCPos = asteriskIndex;
                }
                ...
                currentPos += matchPos + patternSubString.charLength() - 1;
                patternPos = nextWildCPos;
                starPos = -1; // reset star position
            }
            else if (pattern.Buffer[patternPos] == kQuestionMarkSymbol || pattern.Buffer[patternPos] == m_str.Buffer[currentPos])
            {
                patternPos++;
            }
            else
